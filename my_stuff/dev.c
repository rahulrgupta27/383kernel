...
#define Message(a, b...)	printk("[KERN: %s @ %d] :"a"\n", __FUNCTION__, __LINE__, ##b)
				    ...
				    struct sk_buff *skb)
{
	...
	if (!strcmp(dev->name, "eth0") || !strcmp(dev->name, "eth1"))
		Message("KERN: completed q select, return q-id=%d", queue_index);
...
int dev_queue_xmit(struct sk_buff *skb)
{
	...
	if (!strcmp(dev->name, "eth0") || !strcmp(dev->name, "eth1"))
		Message("Calling Tx q select");
	txq = netdev_pick_tx(dev, skb);
	//if (!strcmp(dev->name, "eth0") || !strcmp(dev->name, "eth1"))
		//Message("KERN: Calling Tx q select comlpete");
	...
	if (q->enqueue) {
		if (!strcmp(dev->name, "eth0") || !strcmp(dev->name, "eth1"))
		{
			Message("calling hard xmit: dev=%llx, %s", dev, dev->name);
			uint16_t eth_type; 
